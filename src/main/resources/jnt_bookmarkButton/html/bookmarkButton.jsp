...
<%@ taglib prefix="jnt" uri="http://java.sun.com/jsp/jstl/fmt" %>
...
<c:if test="${renderContext.loggedIn}" >
    <c:if test="${renderContext.liveMode}">
        <form action="<c:url value='${url.baseLive}${renderContext.user.localPath}.add.do'/>" method="post" name="bookmark" id="bookmarkForm">
            <p>
                ...
                <script type="text/javascript">
                    ...
                    var options = {
                        success: function() {
                            $('#bookmardAdded').fadeIn('slow').delay(4000).fadeOut('slow');
        ...
        <div class="hide" id="bookmardAdded">
            <p class="text-success"><fmt:message key='bookmark.added'/></p>
        </div>
    </c:if>
</c:if>
